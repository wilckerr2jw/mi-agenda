...
const TOPICS = [
  ...
  { g: 'Tus datos', t: 'Avisos en el teléfono', when: c => c.cloud,
    b: () => `Ajustes → ${k('Avisos')} → ${k('Recibir avisos en este teléfono')} (debe decir «✓ Este teléfono está registrado»). Por la mañana llega un resumen; durante el día, avisos antes de cada evento (tú eliges cuántos minutos), tareas con hora, reuniones 1 hora antes, «aún no marcaste la lectura de hoy», racha en peligro, cuando alguien de tu familia hace una rutina compartida, lo de mañana por la noche y el recordatorio del informe. Apaga los que no quieras. En las rutinas el aviso trae ${k('✓ Ya lo hice')} para marcarla sin abrir nada. El sonido se elige en el teléfono: mantén presionado un aviso → ⚙️ → Sonido. En iPhone, primero instala la app en la pantalla de inicio.` },
