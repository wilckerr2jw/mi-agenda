...
const TOPICS = [
  ...
  { g: 'Agenda y tareas', t: 'Eventos que se repiten', when: c => c.on('agenda'),
    b: () => `Al crear un evento elige ${k('Cada semana')}, ${k('Cada 2 semanas')} o ${k('Cada mes')}. Para saltarte un día puntual, toca el evento en esa fecha y elige ${k('Cancelar solo el …')}.` },
  ...
  { g: 'Tus datos', t: 'Avisos en el teléfono', when: c => c.cloud,
    b: () => `Ajustes → ${k('Avisos')} → ${k('Recibir avisos en este teléfono')}: un solo aviso al día, a la hora que elijas, con tus tareas de hoy y atrasadas, compromisos, la reunión de mañana (y si falta enviar la agenda) los lunes lo que supervisas, los eventos que te comparten y cuando hay una versión nueva. Por privacidad no muestra títulos, salvo que marques ${k('Mostrar los títulos en el aviso')}. En iPhone, primero instala la app en la pantalla de inicio.` },
